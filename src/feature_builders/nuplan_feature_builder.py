...
class NuplanFeatureBuilder(AbstractFeatureBuilder):
    def __init__(
        ...
        self.history_samples = int(self.history_horizon / sample_interval) #20 过去2s
        self.future_samples = int(self.future_horizon / sample_interval) #80 未来8s
    ...
    def get_features_from_scenario(
        ...
        gtpath_generator=scenario.get_expert_ego_trajectory()
        start_gtpath=next(gtpath_generator)
        last_gtpath = None
        for element in gtpath_generator:
            last_gtpath = element
    ...
    def _build_feature(
        ...
        present_ego_state = ego_state_list[present_idx] #20 刚好是ego cur
        # ego_state_list = (
        #     list(past_ego_trajectory) + [ego_cur_state] + list(future_ego_trajectory)
        # )
        ...
        agent_features = self._get_agent_features(
            query_xy=query_xy, #仅做了distance作用，没有转换自车相对坐标系
            present_idx=present_idx, #判断当前帧
            tracked_objects_list=tracked_objects_list,
        )
        ...
        gt_path=ego_state_list
        cur_pos=present_ego_state
        data["ref_line_info"]=self._get_refline_features(gt_path,query_xy,cur_pos,map_api,route_roadblocks_ids,traffic_light_status,self.radius) #车道中心线
        
        
        return NuplanFeature.normalize(data, first_time=True, radius=self.radius)
    def _get_refline_features(self,gt_path,query_xy,cur_pos,map_api,route_roadblocks_ids,traffic_light_status,radius):
        try:
            route=self._find_route(gt_path,query_xy,cur_pos,map_api,route_roadblocks_ids,traffic_light_status,radius)
        except:
            return None
        ...
        return ref_path
    def _find_route(self,gt_path,query_xy,cur_pos,map_api,route_roadblock_ids,traffic_light_status,radius):
        # 1. get 每个路段的信息 dict 
        ...
        # 2.get 该场景下的start lane、end lane、以及当前的cur lane
        ...
        for lane in lane_objects:
            ...
            #lane的discrete_path是一个稀疏的路段点数组，然后采样得到离散path数组。实际上就是把一个路段的坐标信息进行插值采样，变得更稠密或稀疏。
            # centerline得到的是这个lane片段的中心的一段采样点数组
            ...
            polygon_center[idx] = np.concatenate(
                [
                    centerline[int(sample_points / 2)],# x y 
                    [point_orientation[idx, 0, int(sample_points / 2)]], #heading
                ],
                axis=-1,
            )
    ...
    def _get_ego_current_state(self, ego_state: EgoState, prev_state: EgoState):
        ...
        # print("heading:",ego_state.rear_axle.heading)
        # print("theta:",theta)
        # input()
        ...
        state[6] = yaw_rate
        state[7]=ego_state.dynamic_car_state.rear_axle_velocity_2d.y
        state[8] = ego_state.dynamic_car_state.rear_axle_acceleration_2d.y
        state[9]=ego_state.car_footprint.length
        state[10]=ego_state.car_footprint.width
        state[11]=ego_state.car_footprint.height
        return state
    ...
    def _get_agent_features(
        ...
        N= min(len(present_agents), self.max_agents)
        T= len(tracked_objects_list)
    ...
    def _get_map_features(
        ...
        M= len(lane_objects) + len(crosswalk_objects)
        P=sample_points
