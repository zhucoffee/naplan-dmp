...
class NuplanFeature(AbstractModelFeature):
    ...
    def normalize(
        ...
        # data agent position angle velocity全部根据ego cur state转换为相对自车的相对值。
        # data map的center等也是
        # data["agent"]["target"]是future trajectory与cur trajectory之间的deta值。也就是说，预测的是与当前自车位置的差值。
        # 如果agent valid mask有效，则gt=0，不要比较。
